  ...
  if (Object.entries(oldVoteObject).length === 0) {
    ...
    switch (vote) {
      case "up":
        ...
        const cunt = await incrementVoteOnce(subjectId);
        return cunt;
      case "down":
        console.log("Decrementing vote once");
        const cunt = await decrementVoteOnce(subjectId);
        return cunt;
      default:
    ...
    switch (vote) {
      case "up":
        if (oldVote == vote) {
          ...
          const cunt = await incrementVoteTwice(subjectId);
          return cunt;
        }
      case "down":
        if (oldVote == vote) {
          ...
          const cunt = await decrementVoteTwice(subjectId);
          return cunt;
        }
      default:
        console.log("Error, no matching vote cases!");
