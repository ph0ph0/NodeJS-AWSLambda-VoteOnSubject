  ...
  if (Object.entries(oldVoteObject).length === 0) {
    ...
    switch (vote) {
      case "up":
        ...
        const updatedSubject = await incrementVoteOnce(subjectId);
        return updatedSubject;
      case "down":
        console.log("Decrementing vote once");
        const updatedSubject = await decrementVoteOnce(subjectId);
        return updatedSubject;
      default:
    ...
    switch (vote) {
      case "up":
        if (oldVote == vote) {
          ...
          const updatedSubject = await incrementVoteTwice(subjectId);
          return updatedSubject;
        }
      case "down":
        if (oldVote == vote) {
          ...
          const updatedSubject = await decrementVoteTwice(subjectId);
          return updatedSubject;
        }
        return;
      default:
