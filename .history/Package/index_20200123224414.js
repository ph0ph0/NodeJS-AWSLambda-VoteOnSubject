...
exports.handler = async (event, context, callback) => {
  ...
  const data = await voteExists(voteId, userId, subjectId, vote, voteOn);
  console.log("Data from voteExists: %j", data);

  const voteAlreadyExists = data.voteAlreadyExists;
  const oldVoteObject = data.oldVoteObject;
  console.log("Does vote exist?: %s", voteAlreadyExists);
  console.log("oldVoteObject: %j", oldVoteObject);

  const subject = subjectUpdater(voteAlreadyExists, oldVoteObject, subjectId);
